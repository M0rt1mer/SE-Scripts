...
namespace IngameScript {
    partial class Program : MyGridProgram {
        ...
        Program() {
            ...
            if(TempCollection.Count > 0) { Turret = TempCollection[0] as IMyLargeTurretBase; }
            ...
            if(TempCollection3.Count > 0) { RC = TempCollection3[0] as IMyShipController; }
        }
        #endregion
        ...
        void Main( string argument ) {
            ...
            //Core Block Checks
            if(RC == null || RC.CubeGrid.GetCubeBlock( RC.Position ) == null) { Echo( " ~ No Ship Control Found,\nInstall Forward Facing Cockpit/RC/Flightseat And Press Recompile" ); RC = null; return; }
            if(Turret == null || Turret.CubeGrid.GetCubeBlock( Turret.Position ) == null) { Echo( " ~ Searching For A new Seeker\n (Guidance Turret)\n Name Of Turret Needs to be: '#A#'\nInstall Block And Press Recompile" ); Turret = null; return; }
            if(Alarm == null || Alarm.CubeGrid.GetCubeBlock( Alarm.Position ) == null) { Echo( " ~ No Sound Block For Lock Tone Found,\nInstall Block And Press Recompile\n (script will work fine without) " ); Alarm = null; }

            #endregion

            //Sounds 60 hz growl lock Alarm
            if(Turret != null && Alarm != null) {
                if(!Turret.GetTargetedEntity().IsEmpty() && !Alarm.Enabled) { Alarm.Enabled = true; } else if(Turret.GetTargetedEntity().IsEmpty() && Alarm.Enabled) { Alarm.Enabled = false; }
            }
            ...
            for(int i = 0; i < MISSILES.Count; i++) {
                ...
                if(ThisMissile.IS_CLEAR == true) { STD_GUIDANCE( ThisMissile ); }

                //Fires Straight (NO OVERRIDES)
                else if(ThisMissile.IS_CLEAR == false) {
                    if((ThisMissile.GYRO.GetPosition() - Me.GetPosition()).Length() > ThisShipSize) { ThisMissile.IS_CLEAR = true; }
                }
                ...
                if(Isgyroout || Isthrusterout || Isouttarange) { MISSILES.Remove( ThisMissile ); }
        ...
        void STD_GUIDANCE( MISSILE This_Missile ) {
            ...
            if(LOS_Old.Length() == 0) { LOS_Delta = new Vector3D( 0, 0, 0 ); LOS_Rate = 0.0; } else { LOS_Delta = LOS_New - LOS_Old; LOS_Rate = LOS_Delta.Length() / Global_Timestep; }
        ...
        void PREP_FOR_LAUNCH( int INT ) {
            ...
            for(int i = 0; i < TemporaryThrust.Count; i++) {
                ...
                if(item.CubeGrid != ThisMissile.GYRO.CubeGrid) { ThisMissile.THRUSTERS.Remove( item ); continue; }
            }
            ...
            foreach(IMyThrust item in ThisMissile.THRUSTERS) {
                ...
                if(ThrustDict.ContainsKey( Fwd ) == false) { ThrustDict.Add( Fwd, Thrval ); } else { ThrustDict[Fwd] = ThrustDict[Fwd] + Thrval; }
            }
            ...
            for(int i = 0; i < TemporaryThrust.Count; i++) {
                ...
                if(item.WorldMatrix.Forward != ThrForward) { item.ApplyAction( "OnOff_On" ); ThisMissile.THRUSTERS.Remove( item ); continue; }
            ...
            for(int i = 0; i < ThisMissile.WARHEADS.Count; i++) {
                ...
                if(item.CubeGrid != ThisMissile.GYRO.CubeGrid) { ThisMissile.WARHEADS.Remove( item ); continue; }
            ...
            double number;
            if(double.TryParse( ThisMissile.GYRO.CustomData, out number )) { double.TryParse( ThisMissile.GYRO.CustomData, out ThisMissile.MissileMass ); }
      ...
      ---------------------------------------     */
        ...
        static class RdavUtils {
            ...
            public static bool SphereIntersect_Solv( BoundingSphereD Sphere, Vector3D LineStart, Vector3D LineDirection, out Vector3D Point1, out Vector3D Point2 ) {
                ...
                //Calculates Values
                Double t1, t2;
                if(!Quadractic_Solv( 1, b, c, out t1, out t2 )) { return false; } //does not intersect
                else {
            ...
            public static class DiagTools {
                ...
                public static void Renam_Block_Typ( IMyGridTerminalSystem GTS, string RenameTo ) {
                    ...
                    if(TempCollection.Count < 1) { return; }
                ...
                public static void Renam_Block_Cust( IMyGridTerminalSystem GTS, string customnam, string RenameTo ) {
                    ...
                    if(TempCollection.Count < 1) { return; } else { TempCollection[0].CustomName = RenameTo; }
                }
        ...
        void QuickEcho( object This, string Title = "" ) {
            if(This is Vector3D) { Echo( Title + " " + Vector3D.Round( ((Vector3D)This), 3 ) ); } else if(This is double) { Echo( Title + " " + Math.Round( ((double)This), 3 ) ); } else { Echo( Title + " " + This ); }
        }
